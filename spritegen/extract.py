...
def _asset_prompt(obj: dict, view: str, contents=None) -> str:
    """Subject-side fields, the exclusion clause, then the view phrase.

    No style — the pack prefix supplies that at build time, and repeating it
    here would double it.
    """
    body = vision.subject_prompt(obj)
    phrase = vision.VIEW_POOL.get(view, vision.VIEW_POOL[vision.DEFAULT_VIEW])
    exclude = _exclusion_clause(contents) if contents else ""
    return ", ".join(p for p in (body, exclude, phrase) if p)
    ...
    for obj in objects:
        ...
        for view in obj.get("views") or [vision.DEFAULT_VIEW]:
            ...
            lines += [
                ...
                "prompt    = " + packwriter.toml_string(
                    _asset_prompt(obj, view, contents.get(obj["id"]))),
